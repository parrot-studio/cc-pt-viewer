class AbilityConditions extends React.Component {
  ...
  renderCategoryList() {
    let cs = _.map(this.conditions.abirityCategorys(), (c) => {
  ...
  renderConditionList() {
    ...
    if (category) {
      let cs = _.map(this.conditions.abirityConditionsFor(category), (c) => {
        return <option value={c[0]} key={c[0]}>{c[1]}</option>
      })
  ...
  renderEffectList() {
    let category = this.props.abilitycategory
    if (category) {
      let cs = _.map(this.conditions.abirityEffectsFor(category), (c) => {
        return <option value={c[0]} key={c[0]}>{c[1]}</option>
      })
      return _.concat([<option value={""} key={""}>{"（全て）"}</option>], cs)
    } else {
